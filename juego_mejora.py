...
#Jugadas posibles
######################################################################################################
def Click_jugadas(fila,columna):
    Clave=Matriz[fila][columna]
    Clave=Revisar_String(Clave)
    Dobles=False
    print(Clave)
    #Revisar Vertical
    if Clave[1]==0:
        num=Matriz[fila+1][columna]
        result=0
        while type(num)!=str:
            if fila==8:
                result+=1
                break
            if type(num)==str or num==-2:
                break
            else:
                result+=1
            fila+=1
    ...
    #Revisar Horizontal
        ...
        Clave_V=Clave[0]
        num=Matriz[copy_fila][copy_columna+1]
        result_H=0
        while type(num)!=str:
            print("AA")
            if copy_columna==8:
                ...
                if type(num)==str or num==-2:
                    ...
                    result_H+=1
                copy_columna+=1
                num=Matriz[copy_fila][copy_columna]
        Clave_H=Clave[1]

    if Dobles==True:
        casillas_V=result_V-1
        posibles_jugadas_V=permu(Clave_V,casillas_V)
        
        casillas_H=result_H-1
        posibles_jugadas_H=permu(Clave_H,casillas_H)

        tkinter.messagebox.showinfo("Posibles Jugadas.","Posibles jugadas verticales:\n"+str(posibles_jugadas_V)+"\n"+"Posibles jugadas horizontales:\n"+str(posibles_jugadas_H))
    else:        
        casillas=result-1
        posibles_jugadas=permu(Clave,casillas)
        tkinter.messagebox.showinfo("Posibles Jugadas.","Posibles jugadas:\n"+str(posibles_jugadas))

def permu(num,casillas):
    comb=list(combinations_with_replacement([1,2,3,4,5,6,7,8,9],casillas))
    return permu_aux(num,comb,[])

def permu_aux(num,combinaciones,result):
    if combinaciones==[]:
        return result
    else:
        x=combinaciones[-1]
        if suma(x,0,num,1)==True:
            if suma(x[1:],True,x[0],2)==True:
                return permu_aux(num,combinaciones[:-1],result+[list(x)])
            else:
                return permu_aux(num,combinaciones[:-1],result)
        else:
            return permu_aux(num,combinaciones[:-1],result)

def limitar(comb,cont):
    if cont==501:
        return comb
    else:
        del comb[-1]
        return limitar(comb,cont+1)



def suma(Lista,result,num,modo):
    if modo == 1:
        if Lista==[] or Lista==():
            if result==num:
                return True
            ...
            return suma(Lista[1:],result+Lista[0],num,modo)
    else:
        if Lista==[] or Lista==():
            return result
        else:
            if rev(Lista,num)==False:
                return False
            else:
                return suma(Lista[1:],result,Lista[0],2)
            

def rev(combinaciones,num):
    if combinaciones==[] or combinaciones==():
        return True
    else:
        if combinaciones[0]==num:
            return False
        else:
            return rev(combinaciones[1:],num)
